...
async function manageReservations() {
  try {
    ...
    if (recentReservations.data?.reservations) {
      console.log(`\nFound ${recentReservations.data.total || 0} total reservations`);
      console.log('Most recent reservations:');

      for (const res of recentReservations.data.reservations.slice(0, 5)) {
      ...
      // Manage custom fields for the first reservation
      if (recentReservations.data.reservations.length > 0) {
        ...
        try {
          ...
          // Example: Update custom fields (commented out to avoid making changes)
          console.log('\n(To update custom fields, uncomment the code in the example)');
          /*
          ...
          */
          ...
          console.log('Could not access custom fields:', error.message);
        }
      }
    }

    // Example: Create a test reservation (commented out)
    console.log('\n\n--- Creating Reservations ---');
    console.log('(To create a test reservation, uncomment the code in the example)');
    /*
